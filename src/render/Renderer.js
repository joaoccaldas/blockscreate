...
// Second atlas (assets/generated/textures/blocks_extra.png) covering blocks the
// hand-made atlas didn't, generated by tools/gen-textures.mjs. Keep in sync
// with TEXTURE_LAYOUT there.
const EXTRA_ATLAS = {
...
// grass/dirt/stone reads differently in a dino jungle vs. an industrial waste.
// [r,g,b] multipliers; omitted eras render untinted.
const ERA_TERRAIN_TINT = {
  cell: [0.82, 1.05, 1.02],
  stone: [1.04, 1.03, 0.9],
  bronze: [1.06, 1.0, 0.86],
  iron: [0.95, 0.97, 1.0],
  industrial: [0.82, 0.82, 0.86],
};
// Blocks that should NOT be era-tinted (ores/crafted read by their own color).
const NO_TINT = new Set(['coal_ore', 'copper_ore', 'tin_ore', 'iron_ore', 'gold_ore', 'torch', 'campfire', 'meteor_shard', 'nutrient_blob', 'mineral_vent', 'lipid_membrane']);
...
export class Renderer {
  ...
  render(scene) {
    ...
    for (let ty = y0; ty <= y1; ty++) {
      for (let tx = x0; tx <= x1; tx++) {
        ...
        if (id === AIR) continue;
        this.drawBlock(camera, world, tx, ty, id, T, world.eraId);
      }
    }
  ...
  drawBlock(camera, world, tx, ty, id, T, eraId) {
    const ctx = this.ctx;
    const b = getBlock(id);
    if (!b.colors) return;
    ...
    const tint = (eraId && !NO_TINT.has(b.name)) ? ERA_TERRAIN_TINT[eraId] : null;

    if (this.drawBlockTexture(b, sx, sy, T, dk, tint)) {
      if (b.light) this.drawLightGlow(sx, sy, T);
      return;
    }

    // Flat fallback (no atlas tile): apply era tint to the base colors too.
    const tc = (col) => tint ? [col[0] * tint[0], col[1] * tint[1], col[2] * tint[2]] : col;
    const base = tc(hexToArr(b.colors.base));
    const top = tc(hexToArr(b.colors.top));
    const side = tc(hexToArr(b.colors.side));

    if (b.liquid) {
      const wob = Math.sin(this.t * 2 + tx * 0.6) * T * 0.04;
      ctx.fillStyle = withAlpha(shadeArr(base, dk), 0.78);
      ctx.fillRect(sx, sy + T * 0.15 + wob, T + 1, T * 0.85);
      return;
    }

    ctx.fillStyle = shadeArr(base, dk);
    ctx.fillRect(sx, sy, T + 1, T + 1);

    ctx.fillStyle = shadeArr(top, dk);
    ctx.fillRect(sx, sy, T + 1, Math.max(2, T * 0.18));

    ctx.fillStyle = shadeArr(side, dk * 0.82);
  ...
  drawBlockTexture(block, sx, sy, T, darkness, tint = null) {
    // Prefer the hand-made atlas; fall back to the generated extra atlas.
    let atlas = this.sprites.blockAtlas;
    let pos = BLOCK_ATLAS[block.name];
    if (!pos || !atlas || !atlas.complete || !atlas.naturalWidth) {
      atlas = this.sprites.blockAtlasExtra;
      pos = EXTRA_ATLAS[block.name];
    }
    ...
    // Era terrain tint as a multiply wash (kept subtle).
    if (tint) {
      ctx.globalAlpha = 0.22;
      ctx.globalCompositeOperation = 'multiply';
      ctx.fillStyle = `rgb(${Math.round(tint[0] * 200)},${Math.round(tint[1] * 200)},${Math.round(tint[2] * 200)})`;
      ctx.fillRect(sx, sy, T + 1, T + 1);
...
// Array-color variants, used when an era tint pre-multiplies the base colors.
function hexToArr(hex) { return hexToRgb(hex); }
function shadeArr([r, g, b], f) {
